...
function Load_Products(obj) {
        ...
        for (let i = 0; i < obj.length; i++) {
          ...
          const request = $.ajax({
            type: "POST",
            url: `http://127.0.0.1:8000/Farshtore/Search_view/${Product_Code}`,
            success: function(Read_Likes) {
              const obj_view = JSON.parse(Read_Likes);
              obj_view.forEach((items) => {
                  Like_user_Id = items.user_Id
                  view_Product_Code = items.Product_Code
              });
              if (Product_Code == view_Product_Code )
              {
